...
// 源自 [lsfHook](https://github.com/rifflock/lfshook)，不过没有打印堆栈信息，自己定制
...
func addStack(entry *logrus.Entry) {
	skipFrames := 8
	if len(entry.Data) == 0 { // When WithField(s) is not used
		skipFrames = 10
	}

	frames := Callers(skipFrames, false)
	if strings.HasSuffix(frames[0].File, "logrus/exported.go") || strings.HasSuffix(frames[0].File, "logrus/entry.go") {
		skipFrames++
	...
	if entry.Level == logrus.ErrorLevel {
		frames := Callers(skipFrames, true)
